...
//   author <n> <unix-timestamp>
//   committer <n> <unix-timestamp>
//helslo
//   <commit message>
//
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    // Step 1: Build the tree from the index
    ObjectID tree_id;
    ...
    // Step 3: Try to read the parent commit (fails on first commit — that's OK)
    ObjectID parent_id;
    ...
    // Print the short hash like git does
    char hex[HASH_HEX_SIZE + 1];
